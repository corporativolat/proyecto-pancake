...
23. `supabase-migration-34.sql` — **alineación con Excel "Resumen" operativo**. Trae a la BD la lógica que el equipo usa en el sheet (`1CI0Vbg4...`). Añade a `projects`:
    - `priority` text CHECK `null|'estrella'|'atencion'` — equivalente a la columna ATENCIÓN del Excel (⭐ / ⚠️ / vacío). Index parcial. Trigger SECURITY DEFINER `notify_project_priority_change` emite kind `project_priority_change` a owner + miembros al cambiar.
    - `extra_category_ids uuid[]` (default `'{}'`, índice GIN) — multi-TIPO. El Excel acepta combos ("Alianza comercial, Parametrizaciones"); `category_id` sigue siendo la primaria (back-compat con queries y dashboards).
    - `status` CHECK ampliado: añade `'Cancelado'` (no reemplaza estados existentes; `'Entregado'` y `'Finalizado'` conviven).
    - `blocker_note text` + `blocker_since timestamptz` — registro estructurado del bloqueo activo. Trigger BEFORE `handle_project_blocker` sella/limpia `blocker_since` automáticamente. Trigger AFTER `notify_project_blocker` emite kind `project_blocked` (owner + miembros + activity tag `bloqueo`) cuando aparece el bloqueo, y `project_unblocked` (activity tag `avance`) cuando se resuelve.
    - **Seed idempotente**: los 25 proyectos vivos del Excel (mayo 2026). Solo INSERT WHERE NOT EXISTS por title (case-insensitive) — no toca proyectos editados en producción. Resuelve `owner_id` por nombre del responsable; si no hay match, `owner_label` se rellena con el nombre crudo.

    **Cliente:**
    - `src/lib/utils.js`: `STATUSES` ampliado con `step` (orden numérico Excel) y `label` (con prefijo "0.", "1.", "2.", "4.", "6.", "7." donde aplica) + `'Cancelado'`. Nuevas exports: `STATUS_BY_NAME`, `PRIORITY`, `PRIORITY_OPTIONS`, `daysSinceStart(p)`, `daysToDue(p)`, `projectDurationDays(p)`, `projectAllCategoryIds(p)`, `projectCategoryNames(p, cats)`, `isBlocked(p)`. `isFinalStatus` ahora incluye `'Cancelado'`.
    - Componentes nuevos: `PriorityBadge.jsx` (⭐ / ⚠️ badge editable), `ProjectMetricsRow.jsx` (tira Δ inicio · Vence · Duración), `CategoryChips.jsx` (multi-categoría con picker, promote-to-primary, remove), `BlockerCard.jsx` (textarea de bloqueo con timestamp), `ProjectDetailTabs.jsx` (4 tabs Información / Seguimiento / Operación / Gestión persistidos en URL hash).
    - **Tabla maestra `Projects.jsx`**: nueva columna Atención (⭐/⚠️/🚧 si bloqueado), columna Tipo ahora renderiza `CategoryChips` (multi), columna Inicio muestra Δ "hace Xd" / "en Xd", nueva columna Duración (hidden en móvil), filtro nuevo `Atención` (estrella / atención / bloqueados), sort por priority por default. Búsqueda full-text ahora incluye `blocker_note` y nombres de categorías secundarias. Status dropdown muestra `label` con prefijo numérico.
    - **`ProjectDetail.jsx`**: header compacto siempre visible con PriorityBadge editable + CategoryChips editables + ProjectMetricsRow. Botón "Compactar/Detalles" eliminado (reemplazado por tabs). Botones inline de Docs/Tareas eliminados (movidos al tab Gestión). 4 tabs: **Información** (objetivo, observación, responsable, status, salud, dependencia, fechas, contrato, equipo, BlockerCard), **Seguimiento** (roadmap + gantt + hitos), **Operación** (Comments + ActivityFeed), **Gestión** (ClientDocsPanel inline + ClientTasksPanel inline + ProjectQuestionnairesSection + contrato). Tab persistido en URL hash (#info, #seguimiento, #operacion, #gestion). Default = #seguimiento (sin disruption del flujo actual).
    - **`NotifBell.jsx`**: nuevos kinds mapeados `project_priority_change` (Star/amber), `project_blocked` (AlertOctagon/red), `project_unblocked` (CheckCheck/emerald), `project_due_soon` (CalendarClock/amber), `project_overdue` (AlertCircle/red).
    - **Edge function `notify-deadlines`**: `'Cancelado'` añadido a `FINAL_STATUSES`. Tras enviar el email, además inserta una notif in-app en `notifications` (`kind='project_due_soon'` para 5d/3d/1d; `'project_overdue'` para due/overdue+N) al owner del proyecto, con idempotencia por `meta->>window`.
