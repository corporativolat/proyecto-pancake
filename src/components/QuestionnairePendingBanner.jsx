...
// Banner del portal cliente: avisa que hay cuestionarios pendientes
// (status borrador o rechazado) en project_questionnaires.
export default function QuestionnairePendingBanner() {
  const { profile } = useAuth();
  const navigate = useNavigate();
  const [rows, setRows] = useState([]);
  const [dismissed, setDismissed] = useState(() => {
    try { return sessionStorage.getItem('pending-questionnaire-dismissed') === '1'; } catch { return false; }
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      (qs || []).forEach(q => {
        if (!byProject.has(q.project_id)) byProject.set(q.project_id, []);
        byProject.get(q.project_id).push(q);
      });
      const list = [];
      projects.forEach(p => {
        const arr = byProject.get(p.id) || [];
        arr.forEach(q => list.push({ projectId: p.id, projectTitle: p.title, ...q }));
      });
  ...
  const close = () => {
    try { sessionStorage.setItem('pending-questionnaire-dismissed', '1'); } catch { /* noop */ }
    setDismissed(true);
  };

  const goFirst = () => {
    const first = rows.find(r => r.status === 'rechazado') || rows[0];
    navigate('/portal/projects/' + first.projectId);
  };
      ...
      <div className="p-5 md:p-6 flex flex-col md:flex-row gap-5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[12px] text-violet-800 mb-3 leading-relaxed">
            Tienes <strong className="text-violet-900">{rows.length}</strong> cuestionario(s) por enviar
            ...
            . Completar los cuestionarios ayuda a tu equipo a configurar mejor tu proyecto.
          </p>
          ...
          <ul className="space-y-1.5 mb-4 max-h-40 overflow-y-auto pr-2 scroller">
            {rows.slice(0, 5).map(r => (
              <li key={r.id} className="flex items-center gap-2.5 text-[12px] bg-white/60 backdrop-blur rounded-lg px-3 py-2 border border-violet-100">
                <span className={`w-2 h-2 rounded-full flex-shrink-0 ${r.status === 'rechazado' ? 'bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.5)]' : 'bg-violet-500'}`} />
